    ...
    async ({ sessionId }) => {
      ...
      for (const commit of session.commits) {
        for (const fiber of commit.fibers ?? []) {
          ...
          durationByComponent.set(
            name,
            (durationByComponent.get(name) ?? 0) + (fiber.actualDuration ?? 0),
          );
        }
      }
      ...
      if (session.profilingAvailable === null) {
        ...
        profilingStatus =
          'unavailable — app may be running a production build; switch to `next dev`';
      }

      const diagnosis =
        unknownEvents > 0 && commitCount === 0
